		...
		if (svcs !== undefined) {
			svcs.forEach(function (svc) {
				...
				if (svc.port !== undefined) {
					...
					port = {};
					if (svc.weight !== undefined) {
						svc.weight = parseInt(svc.weight, 10);
						if (!isNaN(svc.weight)) {
							port.weight = svc.weight;
						}
					}
					if (svc.priority !== undefined) {
						svc.priority = parseInt(svc.priority, 10);
						if (!isNaN(svc.priority)) {
							port.priority = svc.priority;
						}
					}
					if (!isNaN(svc.port)) {
